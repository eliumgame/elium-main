...
export default function GroupsPanel() {
  ...
  const openGroup = useCallback(
    async (id: string) => {
      ...
      setCreating(false);
      try {
        const res = (await d.api.getGroup(orgId, id)) as {
          members: GroupMember[];
          myWrappedGroupPrivate: WrappedKey | null;
        };
  ...
  const addMember = async () => {
    ...
    if (!myWrapped) {
      await dialogs.alert({
        title: "Impossible",
        message: "Vous ne faites pas partie de cette équipe (clé indisponible).",
      });
      return;
    }
    try {
  ...
  const removeMember = async (m: GroupMember) => {
    ...
    if (
      !(await dialogs.confirm({
        ...
        message: `Retirer ${m.displayName || m.email} ?`,
            ...
            <div className="field">
              ...
              <div className="dc-chip-input">
                ...
                <input
                  ...
                  placeholder="ajouter@exemple.fr ↵"
                  type="email"
                />
              </div>
            </div>
