...
export default function Ribbon(p: RibbonProps) {
  ...
  return (
    <div className="pdfx-ribbon" role="region" aria-label="Barre d'outils PDF">
      ...
      {/* The ribbon scrolls when it does not fit: chevrons and fades say which side has more commands. */}
          ...
          {p.tab === "forms" && (
            <>
              ...
              <Group title="Créer des champs">
                ...
                <Cmd
                  icon={<Type size={17} />}
                  onClick={T("field:text")}
                  active={p.tool === "field:text"}
                  label="Texte"
                />
                <Cmd
                  icon={<BoxSelect size={17} />}
