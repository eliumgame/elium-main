...
export default function SlidesEditor({ store, chrome }: { store: DeckStore; chrome: SlidesEditorChrome }) {
  ...
  const ungroupSelection = () =>
    selIds.forEach((id, i) => store.updateEl(id, { groupId: undefined }, i === selIds.length - 1));
  const removeSelection = () => {
    const ids = [...selIds];
    ids.forEach((id) => store.removeEl(id));
    setSelIds([]);
  };
  ...
  const addEl = (elm: SlideElement) => {
    store.addEl(elm);
    setSelId(elm.id);
  };
  const alignSlide = (dir: "l" | "c" | "r" | "t" | "m" | "b") => {
    if (!sel) return;
    const p: Partial<SlideElement> = {};
    if (dir === "l") p.x = 2;
    if (dir === "c") p.x = 50 - sel.w / 2;
    if (dir === "r") p.x = 98 - sel.w;
    if (dir === "t") p.y = 2;
    if (dir === "m") p.y = 50 - sel.h / 2;
    if (dir === "b") p.y = 98 - sel.h;
  ...
  const setChart = (patch: Partial<ChartData>) => {
    if (!sel?.chart) return;
    store.updateEl(sel.id, { chart: { ...sel.chart, ...patch } }, true);
  };
  const setPoint = (i: number, label: string, value: number) => {
    if (!sel?.chart) return;
    const labels = sel.chart.labels.slice();
    const values = sel.chart.values.slice();
    labels[i] = label;
    values[i] = value;
  ...
  const addPoint = () => {
    if (!sel?.chart) return;
    setChart({ labels: [...sel.chart.labels, "?"], values: [...sel.chart.values, 0] });
  };
  const delPoint = (i: number) => {
    if (!sel?.chart) return;
    setChart({
      labels: sel.chart.labels.filter((_, k) => k !== i),
      values: sel.chart.values.filter((_, k) => k !== i),
    });
  };
  ...
  const bcDeck = () =>
    chanRef.current?.postMessage({ type: "deck", slides: deck.slides, theme, title: chrome.title } as PresenterMsg);
  const bcPos = (present: boolean) =>
  ...
  const stopPresent = () => {
    setPresenting(false);
    try {
      chanRef.current?.postMessage({ type: "end" } as PresenterMsg);
    } catch {
      /* ignore */
    }
    try {
  ...
  useEffect(() => {
    ...
    ch.onmessage = (e: MessageEvent) => {
      ...
      if (m.type === "ready") {
        ...
        if (m.dir === "next") navRef.current.next();
  ...
  useEffect(() => {
    if (presenterOn) bcDeck();
  }, [presenterOn, deck.slides, theme]); // eslint-disable-line react-hooks/exhaustive-deps
  useEffect(() => {
    if (presenterOn) bcPos(presenting);
  }, [presenterOn, presentIdx, presentStep, presenting]); // eslint-disable-line react-hooks/exhaustive-deps
  useEffect(
    () => () => {
      try {
        winRef.current?.close();
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      if ((e.ctrlKey || e.metaKey) && !typing) {
        ...
        if (k === "z" && !e.shiftKey && store.undo) {
          ...
          e.preventDefault();
          store.redo();
        } else if (k === "c" && selIds.length) {
          e.preventDefault();
          copySelection();
        } else if (k === "x" && selIds.length && canWrite) {
          e.preventDefault();
          copySelection();
          removeSelection();
        } else if (k === "v" && canWrite) {
          e.preventDefault();
          pasteClipboard();
        } else if (k === "d" && selIds.length && canWrite) {
          e.preventDefault();
          duplicateSelection();
        } else if (k === "g" && canWrite) {
          e.preventDefault();
          if (e.shiftKey) ungroupSelection();
          else groupSelection();
        } else if (k === "a" && elements.length) {
          e.preventDefault();
          setSelIds(elements.map((el) => el.id));
        }
      }
      if ((e.key === "Delete" || e.key === "Backspace") && selIds.length && !typing && canWrite) {
        e.preventDefault();
        removeSelection();
      }
    };
        ...
        (() => {
          ...
          return (
            <div className="present-overlay">
              <button className="present-close icon-btn" title="Quitter (Échap)" onClick={stopPresent}>
                <X size={22} />
              </button>
              <button className="present-arrow present-arrow--prev icon-btn" onClick={goPrev} disabled={atStart}>
                <ChevronLeft size={28} />
              </button>
              <div className="present-stage" onClick={goNext}>
                {cur && (
                  <div
                    key={morphing ? `m${presentIdx}` : presentIdx}
                    className={`present-anim sv-present-canvas ${trans !== "none" && trans !== "morph" ? `pt-${trans}` : ""}`}
                  >
                    {morphing ? (
                      <MorphCanvas
                        prev={deck.slides[morphFrom]!}
                        next={cur}
                        theme={theme}
                        scale={presentScale}
              ...
              <button className="present-arrow present-arrow--next icon-btn" onClick={goNext} disabled={atEnd}>
                <ChevronRight size={28} />
              </button>
              <div className="present-nav">
                {presentIdx + 1} / {deck.slides.length}
                {steps > 0 ? ` · ${presentStep}/${steps}` : ""}
              </div>
            </div>
          );
        })()}
