...
function paragraph(
  ...
  sz: number,
  color: string,
  opts: { bullet?: boolean; bold?: boolean; align?: string } = {},
): string {
  const pPr = `<a:pPr${opts.align ? ` algn="${opts.align}"` : ""}${opts.bullet ? ' marL="285750" indent="-285750"' : ""}>${opts.bullet ? '<a:buFont typeface="Arial"/><a:buChar char="•"/>' : "<a:buNone/>"}</a:pPr>`;
...
function shapeXml(s: Shape, id: number): string {
  ...
  if (s.kind === "line" || s.kind === "arrow") {
    ...
    return (
      `<p:cxnSp><p:nvCxnSpPr><p:cNvPr id="${id}" name="Connecteur ${id}"/><p:cNvCxnSpPr/><p:nvPr/></p:nvCxnSpPr>` +
  ...
  const fill =
    s.fill === "transparent" ? "<a:noFill/>" : `<a:solidFill><a:srgbClr val="${hex(s.fill)}"/></a:solidFill>`;
  const ln =
    lnW > 0
      ? `<a:ln w="${lnW}"><a:solidFill><a:srgbClr val="${hex(s.stroke)}"/></a:solidFill></a:ln>`
      : "<a:ln><a:noFill/></a:ln>";
  const txt = s.text
    ? `<a:p><a:pPr algn="ctr"/><a:r><a:rPr lang="fr-FR" sz="1400" dirty="0"/><a:t>${xmlEsc(s.text)}</a:t></a:r></a:p>`
    : "<a:p/>";
  return (
    `<p:sp><p:nvSpPr><p:cNvPr id="${id}" name="Forme ${id}"/><p:cNvSpPr/><p:nvPr/></p:nvSpPr>` +
...
function elementXml(
  ...
  if (el.type === "chart" && el.chart) {
    ...
    return (
      `<p:graphicFrame><p:nvGraphicFramePr><p:cNvPr id="${id}" name="Graphique ${id}"/><p:cNvGraphicFramePr/><p:nvPr/></p:nvGraphicFramePr>` +
      `<p:xfrm><a:off x="${x}" y="${y}"/><a:ext cx="${w}" cy="${h}"/></p:xfrm>` +
  ...
  if (el.type === "shape") {
    ...
    const fill =
      (el.fill ?? "transparent") === "transparent"
        ? "<a:noFill/>"
        : `<a:solidFill><a:srgbClr val="${hex(el.fill!)}"/></a:solidFill>`;
    const ln =
      lnW > 0
        ? `<a:ln w="${lnW}"><a:solidFill><a:srgbClr val="${stroke}"/></a:solidFill></a:ln>`
        : "<a:ln><a:noFill/></a:ln>";
    const adj =
      kind === "roundRect"
        ? `<a:avLst><a:gd name="adj" fmla="val ${Math.round(((el.radius ?? 12) / 100) * 100000)}"/></a:avLst>`
        : "<a:avLst/>";
    const txt = el.text
      ? `<a:p><a:pPr algn="ctr"/><a:r><a:rPr lang="fr-FR" sz="1400" dirty="0"/><a:t>${xmlEsc(el.text)}</a:t></a:r></a:p>`
      : "<a:p/>";
    return (
      `<p:sp><p:nvSpPr><p:cNvPr id="${id}" name="Forme ${id}"/><p:cNvSpPr/><p:nvPr/></p:nvSpPr>` +
...
function slideXml(
  slide: Slide,
  colors: { bg: string; title: string; body: string },
  media: { name: string; bytes: Uint8Array }[],
): string {
  ...
  if (slide.layout === "image-full") {
    ...
    if (slide.title)
      parts.push(
        textBox(
          ...
          ex(84, CX),
          ex(12, CY),
          paragraph(slide.title, 2000, colors.title, { bold: true, align: "ctr" }),
          "ctr",
        ),
      );
    ...
    parts.push(
      textBox(
        ...
        "Titre",
        ex(8, CX),
        centered ? ex(34, CY) : ex(7, CY),
        ex(84, CX),
        ex(20, CY),
        paragraph(slide.title || " ", centered ? 4400 : 4000, colors.title, {
          bold: true,
          align: centered ? "ctr" : "l",
        }),
        centered ? "ctr" : "t",
      ),
    );
    if (paras.length)
      parts.push(
        textBox(
          ...
          "Contenu",
          ex(8, CX),
          centered ? ex(56, CY) : ex(30, CY),
          ex(84, CX),
          centered ? ex(20, CY) : ex(62, CY),
          paras
            .map((p) =>
              bodyParagraph(p.html, centered ? 2400 : 2000, colors.body, {
                bullet: p.bullet && !centered,
                align: centered ? "ctr" : "l",
              }),
            )
            .join(""),
